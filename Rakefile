...
S3_BUCKET_NAME  = "heroku-buildpack-ruby-perf"
...
def build_ruby_command(name, output, prefix, usr_dir, tmpdir, perf, rubygems = nil)
  build_command = [
    # need to move libyaml/libffi to dirs we can see
    "mv #{usr_dir} /tmp"
  ]

  build_command << "autoconf" if perf

  build_command << "./configure --enable-load-relative --disable-install-doc --prefix #{prefix}"
  build_command << "env CPATH=/tmp/#{usr_dir}/include:\\$CPATH CPPATH=/tmp/#{usr_dir}/include:\\$CPPATH LIBRARY_PATH=/tmp/#{usr_dir}/lib:\\$LIBRARY_PATH make"
...
task "ruby:install", :version, :perf do |t, args|
  ...
  Dir.mktmpdir("ruby-") do |tmpdir|
    Dir.chdir(tmpdir) do |dir|
      ...
      build_ruby_command(full_name, name, prefix, usr_dir, tmpdir, perf, rubygems)

      # build ruby
      if major_ruby == "1.8"
        output  = "ruby-build-#{version}"
        prefix  = "/app/vendor/ruby-build-#{version}"
        build_ruby_command(full_name, output, prefix, usr_dir, tmpdir, perf, rubygems)
      end
    end
  end
end
